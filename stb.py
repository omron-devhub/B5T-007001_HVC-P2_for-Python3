...
class STB(object):
    ...
    def __init__(self, library_name, exec_func):
        ...
        # Load shared library
        if SysArch == '64bit':
            self.stbDLL = OkaoSTB(os.path.join('./x64/', library_name))
            self.hSTB = cast(self.stbDLL.STB_CreateHandle(exec_func), c_void_p)
        elif SysArch == '32bit':
            self.stbDLL = cdll.LoadLibrary(os.path.join('./x32/', library_name))
            self.hSTB = self.stbDLL.STB_CreateHandle(exec_func)
        else:
            print("Unsupported architecture. Exiting application . . .")
            exit()
